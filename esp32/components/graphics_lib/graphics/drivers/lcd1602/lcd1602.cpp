...
void lcd1602_driver::send_command(const bool rs, const uint8_t value) const
{
...
void lcd1602_driver::send_nibble(const uint8_t nibble, const bool rs) const
{
...
void lcd1602_driver::set_cursor(uint8_t col, uint8_t row) const
{
    if (col > 15) col = 15;
    if (row > 1) row = 1;
...
lcd1602_driver::lcd1602_driver(const i2c_master_bus_handle_t bus_handle, const uint8_t address)
{
    ...
    ESP_LOGI("lcd1602_driver", "Device configured");
}

void lcd1602_driver::init()
{
    ...
    ESP_LOGI("lcd1602_driver", "Device initialized");
}

void lcd1602_driver::clear()
{
    send_command(false, 0x01);
    esp_rom_delay_us(2000);
}

void lcd1602_driver::draw(const std::vector<std::string> rows)
{
    uint8_t i = 0;
    for (auto& row : rows)
    {
...
void lcd1602_driver::write(const std::string& str) const
{
